---
title: 'TypeScript #1: La Inferència treballa per tu'
...
Aquesta és la primera part d'una sèrie que intentarà que entenguis TypeScript _**de veritat**_, no només perquè copiïs i enganxis "any" quan el compilador es queixa.

L'objectiu de la sèrie completa és acabar comprenent que TypeScript no és només JavaScript amb tipus, sinó que TypeScript és un protolenguatge de tipat per crear tota estructura de tipus que necessitis per als teus projectes i fer que, encara que sí, tot sigui més lent i pesat amb les maleïdes 👹 advertències, al final guanyis en seguretat i robustesa de codi.

Jo no soc cap expert, però sempre he pensat que per aprendre el millor és tractar d'ensenyar als altres. Això t'obliga a abordar cada tema de manera que qualsevol pugui entendre'ls i veure-ho des de moltes perspectives.

Per això, en aquesta sèrie intentaré explicar cada concepte des de zero, sense utilitzar termes tècnics complexos, sense utilitzar llenguatge tècnic que només els que ja saben TypeScript entenguin.
...
<Callout type="info" title="Info" filled>
    Nota: Aquesta és la primera entrada d'una sèrie de TIL sobre aprenentatge. Encara que és diferent de la resta de TIL que són notes curtes, he pensat a reutilitzar-lo perquè al cap i a la fi segueix la mateixa idea d'alguna cosa que aprens dia a dia. Per ara això és un experiment.
</Callout>
...
TypeScript ja sap que "Denis" és un string. No cal que l'hi juris.
...
const userName = "Denis"; // TS infereix: string (o literal "Denis", veurem això després)
