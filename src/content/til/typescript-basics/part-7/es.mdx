...
Este patrón lo verás en todas partes. Queremos una función que obtenga una propiedad de un objeto de forma segura.

Necesitamos definr dos Generics:
1. `T`: El objeto.
...
<CodeTabs items={[
    {
		label: 'TypeScript',
		code: `// Si no me pasas T, asumiré que es 'string'
...
Crea una función `fusionar` que tome dos objetos `obj1` y `obj2` y los combine. Tipa la función usando dos generics `T` y `U`.
