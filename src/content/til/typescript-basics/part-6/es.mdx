...
Imagina una función que devuelve lo que le pasas (clásica función `identity`).
...
Podemos decirle a la función: *"Oye, voy a pasarte un tipo al que llamaremos `T`. Úsalo para tipar el argumento y el retorno"*.
...
La `T` es una convención (viene de *Type*), pero puedes llamarlo como quieras: `<Data>`, `<Response>`, `<Props>`.
...
Si has usado React, ya has usado Generics sin saberlo.
...
Crea una función `envolver` que tome un valor de tipo `T` y lo devuelva dentro de un array `[T]`.
