...
Son Generics que toman un tipo y te devuelven una versión modificada.

## `Partial<T>` (Todo opcional)

Imagina que tienes un usuario completo, pero para una actualización (PATCH), solo envías los campos que cambiaron.
...
A veces tienes una interfaz gigante y solo necesitas un par de campos para un componente pequeño.
...
Cuando quieres un objeto donde las claves son, por ejemplo, IDs, y los valores son Usuarios.
...
// 1. Define 'TodoUpdate' usando Partial
type TodoUpdate = any; 

// 2. Define 'TodoPreview' usando Pick (solo title y completed)
