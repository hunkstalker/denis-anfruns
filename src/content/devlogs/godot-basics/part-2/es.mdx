...
> [Descargar asset](/assets/godot-basics/shot.png) (Click derecho -> Guardar imagen)
...
Imagina que tu nodo `VisibleOnScreenNotifier2D` es un **Vigía**. Su trabajo es gritar **"¡He salido de la pantalla!"**.
Nosotros (el script) conectaremos un "cable" a ese grito para reaccionar.
...
2.  **`Input` (El Oído)**: Es quien escucha tu teclado. `is_action_just_pressed` devuelve `true` (verdad) solo en el instante exacto que bajas el dedo. Ideal para disparos tiro a tiro.
...
### El Problema del Canguro

**¡Prueba el juego ahora (F5)!**
Muévete con la nave mientras disparas ráfagas. ¿Notas algo raro?
Las balas se mueven contigo. Si giras o avanzas, las balas te siguen como si estuvieran pegadas con una cuerda invisible.

Esto ocurre porque la bala es **hija de la nave**. Hereda su movimiento.
Es como un bebé canguro en la bolsa de su madre: si la madre salta, el bebé salta con ella.
