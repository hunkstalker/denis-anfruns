...
Anem a crear una bala que sàpiga moure's i morir quan surti de la pantalla.
...
> [Descarregar asset](/assets/godot-basics/shot.png) (Clic dret -> Desa la imatge)
...
2.  Node arrel: **`Area2D`** (Ull! No volem físiques complexes com xocar i rebotar, només detectar si *toquem* alguna cosa). Anomena'l `Bullet`.
3.  Fill: **`CollisionShape2D`** (amb forma de rectangle o càpsula petita).
4.  Fill: **`Sprite2D`** (utilitza un rectangle petit o la teva imatge de bala, i posa-la a la carpeta sprites).
...
Abans de programar el moviment, assegurem-nos que la bala s'esborri en sortir de pantalla.
Per això usarem una **Senyal**.

Imagina que el teu node `VisibleOnScreenNotifier2D` és un **Guaita**. La seva feina és cridar **"He sortit de la pantalla!"**.
Nosaltres (l'script) connectarem un "cable" a aquest crit per reaccionar.
...
4.  Fes doble clic a la senyal `screen_exited`.
5.  Dóna-li a **Connect**.
...
# Aquesta funció l'ha creat Godot automàticament en connectar la senyal.
...
*   **La Instància**: És una **còpia viva** d'aquesta escena. El joc crea aquestes còpies per utilitzar-les (una còpia per al jugador, 10 còpies per a bales, etc.).
...
2.  **`Input` (L'Oïda)**: És qui escolta el teu teclat. `is_action_just_pressed` retorna `true` (veritat) només a l'instant exacte que baixes el dit. Ideal per a trets tret a tret.
...
func _physics_process(delta):
    ...
    if Input.is_action_just_pressed("ui_accept"): # Barra espaiadora
        ...
        add_child(bullet) # <--- L'ERROR (Cangur)`
    }
  ]}
/>

### El Problema del Cangur

**Prova el joc ara (F5)!**
Mou-te amb la nau mentre dispares ràfegues. Notes alguna cosa rara?
Les bales es mouen amb tu. Si gires o avances, les bales et segueixen com si estiguessin enganxades amb una corda invisible.

Això passa perquè la bala és **filla de la nau**. Hereta el seu moviment.
És com un nadó cangur a la bossa de la seva mare: si la mare salta, el nadó salta amb ella.
  ...
  items={[
    {
      label: 'player.gd',
      lang: 'gdscript',
      code: `# Correcte: Utilitzant el pare (L'Escena World)
    if Input.is_action_just_pressed("ui_accept"):
        var bullet = bullet_scene.instantiate()
        
        # 1. Afegir al "Pare" (el Món) PRIMER
        # Necessitem que la bala tingui un pare perquè pugui calcular les seves coordenades globals
        get_parent().add_child(bullet)
         
        # 2. COL·LOCAR
        # Ara que està al món, li diem "posa't on sóc jo"
        bullet.global_position = global_position`
    }
  ]}
/>
A la línia `bullet.global_position = global_position`:
...
Estem dient: *"Bala, col·loca't al món EXACTAMENT on sóc jo (la Nau) ara mateix"*.

Si utilitzéssim `bullet.position = position`, li estaríem dient *"Bala, posa't en relació al Món (pare de la bala) igual que jo estic en relació al Món"*. Si la nau està a (100, 100), la bala apareixeria a (100, 100). Això sembla correcte!

Utilitzar sempre **Global** evita mals de cap: igualem coordenades universals.
...
2.  Mou-lo visualment fins a la punta del canó. Anomena'l `Muzzle` (Boc).
3.  Al codi, utilitza la posició d'aquest marcador:
  ...
  items={[
    {
      label: 'player.gd',
      lang: 'gdscript',
      code: `# En lloc d'utilitzar la meva posició, uso la del marcador
...
Ara la bala sortirà sempre del lloc correcte, encara que canvies la nau!
...
*(Nota: El nom de la funció pot variar segons com anomenis al teu node Notifier/Enabler. Godot ho genera automàticament en connectar la senyal).*
...
El "Recolector d'Escombraries" (Garbage Collector) de la CPU se saturarà netejant i creant memòria, causant estrebades (micro-stuttering).
...
4.  **`queue_free()`**: La forma segura d'esborrar escombraries.

Felicitats! Has creat la teva primera arma semi-funcional. Per ara ho podem disparar, però no danya res.

En el **proper capítol**, anemma donar-los alguna cosa a la que disparar.
