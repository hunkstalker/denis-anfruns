...
Many devs coming from strongly typed languages (like older C, Java, or C#) are naturally accustomed to the idea of typing everything.
...
<CodeTabs
	...
	items={[
		{
			label: 'example.ts',
			code: `// ❌ Redundant (Unnecessary Boilerplate)
...
TypeScript already knows that "Alice" is a string. You don't have to swear it.
...
<CodeTabs
	...
	items={[
		{
			label: 'example.ts',
			code: `// ✅ Perfect (Clean and readable)
...
From our point of view, we would let TS infer any (or put it ourselves) in cases where we don't know what is going to happen, whether due to errors or because the response can be very broad.
...
<CodeTabs items={[
  {
    label: 'TypeScript',
    code: `// ❌ TS cannot read your mind nor know who will call this
...
In **Part 2**, we will look at how to use those literal types to our favor with **Unions**, and how to tell TS: _"This variable can only be 'admin' or 'user', nothing else"_.

## Experiment!

Here you have the starting point. Copy the code and run it in <a href="https://www.typescriptlang.org/play" target="_blank" rel="noopener noreferrer">official TypeScript Playground</a>:
