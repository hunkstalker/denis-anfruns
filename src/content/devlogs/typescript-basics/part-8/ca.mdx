...
Potser has pensat: *"¿I si faig servir `string | number` en lloc de `<T>`?"*

```ts
function echo(val: string | number): string | number { return val; }
const res = echo("Hola"); // TS creu que 'res' és 'string | number' ❌
// Has perdut la precisió: TS ja no sap que és un string.
```

Amb `<T>`, TS manté la connexió: entra string -> surt string.

---

### Provem-ho!

Crea una funció `embolicar` que **posis el que posis**, t'ho retorni dins d'un **array**.
Si poses `10`, surt `[10]`. Si poses `"hola"`, surt `["hola"]`.

```ts
// Repte: Crea la funció genèrica 'embolicar'
// Ha de rebre un valor de tipus T i retornar un array de T (T[])

function embolicar... // Implementa aquí la teva solució

// 🧪 Proves (No toquis això):
const n = embolicar(10);
const s = embolicar("hola");
...
A la **Part 9**, pujarem de nivell. Veurem com **restringir** aquests Generics (perquè no valgui "qualsevol cosa") i com donar-los valors per defecte.
