...
You might have thought: *"What if I use `string | number` instead of `<T>`?"*

```ts
function echo(val: string | number): string | number { return val; }
const res = echo("Hello"); // TS thinks 'res' is 'string | number' ❌
// You lost the precision: TS no longer knows it's a string.
```

With `<T>`, TS keeps the connection: it enters string -> it comes out string.

---

### Let's try it!

Create a function `wrap` that **whatever you put in**, returns it inside an **array**.
If you put `10`, `[10]` comes out. If you put `"hello"`, `["hello"]` comes out.

```ts
// Challenge: Create the generic function 'wrap'
// It must receive a value of type T and return an array of T (T[])

function wrap... // Implement your solution here
...
In **Part 9**, we'll level up. We'll see how to **restrict** those Generics (so that "anything goes" doesn't apply) and how to give them default values.
