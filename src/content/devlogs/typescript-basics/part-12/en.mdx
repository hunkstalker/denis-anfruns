...
<Callout type="tip" title='"Type Functions"'>
	...
	*   `IsString<T>` is the **function** (and `T` the parameter).
...
// TS inferred what was INSIDE the promise and called it 'U'.`,
		lang: 'typescript'
    }
]} />
...
Let's create an `UnpackArray<T>` type that extracts the type from an array. `UnpackArray<string[]>` should return `string`.

<Callout type="tip" title="Why does it return string and not array?">
  Look at the `extends (infer U)[]` condition.
  We're asking: *"Is `T` an Array of something (`U`)?"*.

  If `T` is `string[]`, then that "something" (`U`) is `string`.
  We're extracting the type of data that's **inside** the array!
</Callout>
...
// Check if it works (hover over Unpacked)
// It should be 'string'
```
...
In the **final part** (13), we'll put it all together. We'll see real design patterns, function overloads, and how to create your own "Type-Safe" mini-library.
