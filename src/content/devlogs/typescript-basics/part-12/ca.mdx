...
<Callout type="tip" title='"Funcions de Tipus"'>
	...
	*   `EsString<T>` és la **funció** (i `T` el paràmetre).
...
<CodeTabs items={[
    {
		label: 'exemple.ts',
		code: `// Tenim això:
type RespostaPromesa = Promise<{ id: number; name: string }>;
...
// TS va inferir el que hi havia DINS de la promesa i el va anomenar 'U'.`,
		lang: 'typescript'
    }
]} />
...
Creem un tipus `UnpackArray<T>` que n'extregui el tipus d'un array.
`UnpackArray<string[]>` hauria de retornar `string`.

<Callout type="tip" title="Per què retorna string i no array?">
  Fixeu-vos en la condició `extends (infer U)[]`.
  Li estem preguntant: *"És `T` un Array d'alguna cosa (`U`)?"*.
  
  Si `T` és `string[]`, aleshores aquesta "cosa" (`U`) és `string`.
  Estem extraient el tipus de dada que hi ha **dins** de l'array!
</Callout>
...
// Comprova si funciona (hover sobre Unpacked)
// Hauria de ser 'string'
```
...
A la **Part 13** (i final), ho posarem tot junt. Veurem patrons de disseny reals, sobrecàrrega de funcions i com crear la teva pròpia mini-llibreria "Type-Safe".
