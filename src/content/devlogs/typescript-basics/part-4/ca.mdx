...
<CodeTabs items={[
    {
		label: 'React (Exemple)',
		code: `// useState retorna una tuple: [valor, funció]
...
Són útils quan volem retornar múltiples valors d'una funció sense crear un objecte.

*   Coordenades: `[x, y]` -> `[number, number]`
*   Rangs: `[inici, fi]` -> `[number, number]`
*   Estats: `[loading, error, data]` -> `[boolean, Error | null, any]`
...
Fantàstic per a programació funcional i evitar efectes secundaris!

## Provem-ho!

Embrutem-nos les mans. Tens dos reptes a sota: un per Arrays i un altre per Tuples. Prova les solucions a <a href="https://www.typescriptlang.org/play" target="_blank" rel="noopener noreferrer">TypeScript Playground oficial</a> per veure els errors.
...
```ts
// 1. Crea un array de strings anomenat 'paises'
// const paises...
...
```ts
// Crea la tuple 'usuario' amb tipus [string, number, boolean]
// const usuario...
```
