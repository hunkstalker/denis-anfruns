...
<CodeTabs
  items={[
    {
      label: 'main.ts',
      code: `type Estat = "Carregant" | "Èxit" | "Error";
...
*   **`unknown`**: "No sé què és això, així que t'obligaré a comprovar-ho". (Segur).
*   **`never`**: "Això no hauria de passar mai". (Útil per assegurar lògica exhaustiva).
