...
<CodeTabs items={[
    ...
    verd: "#00FF00",
    blau: "#0000FF"
} as const; // 'as const' fa que siguin literals i readonly

// Volem un tipus que només permeti "vermell" | "verd" | "blau"
type NomColor = keyof typeof COLORS;

function pinta(color: NomColor) { ... }
...
<Callout type="tip" title="Nota Pro" filled>
    El `as const` és vital aquí. Si no el posem, TS inferirà que les propietats són `string`, i `typeof` serà molt genèric. Amb `as const` li diem: "Això no canvia, és literal!".
</Callout>
...
A la **Part 6** ho posarem en joc amb els **Type Guards & Narrowing**: veurem com utilitzar `typeof` i `instanceof` dins del cos d'una funció per estretar el tipus i saber exactament què rebem.
