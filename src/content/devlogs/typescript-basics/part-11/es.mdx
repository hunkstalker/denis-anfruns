...
1. Crea `TodoUpdate` usando `Partial` de `Todo`.
2. Crea `TodoPreview` usando `Pick` de `Todo` (solo `title` y `completed`).
3. Crea `TodoDictionary` que sea un `Record` donde la clave es `string` y el valor es `Todo`.
...
En la **Parte 12**, entraremos en territorio avanzado. Usaremos `infer` para que TypeScript "extraiga" tipos de lugares imposibles, como el tipo de retorno de una función.
