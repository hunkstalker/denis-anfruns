...
In the previous chapter, we made enemies die and fall. It works, but there's no feedback when they take damage.
...
When you write a Shader, you're giving direct instructions to the GPU about how to paint each pixel.
...
Each core can execute complex and different instructions. But even with 28 threads, if you need to process 10,000 pixels, the CPU would have to distribute them among these threads and execute them in **sequential batches**.

### GPU: Thousands of Cores, Very Simple

A modern GPU like the **NVIDIA RTX 4080** has:
*   **9,728 CUDA cores**.
*   Each core is much simpler than a CPU core.
...
If your enemy sprite is 100x100 pixels, that's **10,000 pixels** total.
...
*   Other types exist like `spatial` (3D) or `particles`, but in this course we'll use `canvas_item`.

### `fragment()`

It's the shader's main function. It runs **once for each pixel** of the sprite.
...
It's the mandatory **output variable**. It's where you store the final color the pixel will have.
  ...
  items={[
    {
      ...
      lang: 'glsl',
      code: `void fragment() {
    COLOR = vec4(1.0, 0.0, 0.0, 1.0); // Paint ALL the sprite solid red
}`
    }
  ]}
/>
...
*   The thread for the center pixel would have `UV = (0.5, 0.5)`.

**Technical note**: `UV` is to shaders what `self.position` is to scripts. Each thread knows its own coordinate.

### `texture()`

It's a function that **reads the color of a pixel** from an image given a coordinate.
...
*   Result: Returns the `vec4` (RGBA color) of the pixel at that position in the image.
...
4.  Click on the new material and on `Shader`, select `New Shader`.
...
void fragment() {
    // Read the original color of the sprite at this coordinate (UV)
    vec4 original_color = texture(TEXTURE, UV);
    
    // Assign that color as output
...
If we used `vec4(1.0, 1.0, 1.0, 1.0)`, we would paint a solid white square (including the transparent areas of the sprite).
...
The shader is ready, but it doesn't do anything because `flash_active` is always `false`.
Now we go back to GDScript (`enemy.gd`) to activate it when we take damage.

**Phase 1: Access the Material**

First we need a reference to the sprite's `ShaderMaterial`:
  ...
  items={[
    {
      ...
      lang: 'gdscript',
      code: `func hit_flash():
    # 1. Get the Sprite2D node
    var sprite = $Sprite2D
    
    # 2. Access its material
    var shader_material = sprite.material`
    }
  ]}
/>

**Note**: `sprite.material` returns a generic `Material` type. To use `ShaderMaterial`-specific functions, we need to verify the type first.
...
<CodeTabs
  items={[
    ...
    if shader_material is ShaderMaterial:
        # Activate flash (send 'true' to the GPU)
        shader_material.set_shader_parameter("flash_active", true)`
    }
  ]}
/>

**What is `if shader_material is ShaderMaterial`?**
This is called **type narrowing**:
1. **Verifies** at runtime if `shader_material` is actually a `ShaderMaterial`.
2. **Inside the `if` block**, GDScript knows that `shader_material` is a `ShaderMaterial`, enabling autocompletion of `set_shader_parameter()`.

It's safer than using `as ShaderMaterial`, which blindly assumes the type without checking.
...
*   When calling this, the GPU's 10,000 threads receive `true` and paint white.
...
<CodeTabs
  items={[
    ...
    # Verify that the material is a ShaderMaterial
    if shader_material is ShaderMaterial:
        # Activate flash
        shader_material.set_shader_parameter("flash_active", true)
...
`await` is a keyword that **pauses the execution of this specific function** until something happens (in this case, until the timer's time passes).
...
Only THIS function (`hit_flash`) is "sleeping" waiting. When the timer finishes, the function "wakes up" and continues from where it left off (deactivating the flash).

It's like setting a reminder on your phone: "In 0.1 seconds, turn off the flash". Meanwhile, you go on with your life.
...
5.  **`COLOR`**: Output variable that defines the final color of the pixel.
...
In the **next chapter**, we will create the **UI and basic Game Loop** (score, lives, Game Over).
