...
> Pots duplicar l'escena de l'helicòpter de la secció anterior (anomena-la `enemy_orbital.tscn`) i canviar-li l'script, o simplement canviar l'script a la mateixa escena per provar.
...
> **Tip:** No necessites buscar un altre sprite. Pots duplicar la teva escena de l'helicòpter (`enemy_orbital.tscn`) i reemplaçar el seu script per aquest nou.
...
1.  Obre `world.tscn`.
2.  Duplica la teva escena `enemy.tscn` com a `enemy_wave.tscn`.
3.  Modifica l'script amb el codi d'ondulació.
4.  Arrossega `enemy_wave.tscn` al viewport.
