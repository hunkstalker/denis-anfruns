...
![Sprite del Helicóptero](/assets/godot-basics/chopper_128.png)
...
func _on_area_entered(area):
  ...
  func hit_flash():
    # El mismo código de flash-shader de impacto ...
    
  func die():
    # El mismo código de muerte ...`
    }
  ]}
/>
...
> Puedes duplicar la escena del helicóptero de la sección anterior (llámala `EnemyOrbital.tscn`) y cambiarle el script, o simplemente cambiar el script en la misma escena para probar.
...
> **Tip:** No necesitas buscar otro sprite. Puedes duplicar tu escena del helicóptero (`EnemyOrbital.tscn`) y reemplazar su script por este nuevo.
...
@export var speed = 200
@export var hp = 3
@export var radius_x = 150.0  # Radio horizontal (más ancho)
@export var radius_y = 50.0   # Radio vertical (más estrecho)
...
func _process(delta):
    if is_dying:
      # Aplicamos gravedad sobre la velocidad que traíamos
      velocity.y += 1000 * delta
      position += velocity * delta
    else:
      var prev_pos = position # Guardamos posición antes de mover
      time += delta
      ...
      # (nuevo) Calculamos la velocidad real instantánea (Derivada)
      # Esto nos da la dirección exacta en la que se movía
      velocity = (position - prev_pos) / delta
...
1.  Abre `world.tscn`.
2.  Duplica tu escena `Enemy.tscn` como `EnemyWave.tscn`.
3.  Modifica el script con el código de ondulación.
4.  Arrastra `EnemyWave.tscn` al viewport.
