...
> You can duplicate the chopper scene from the previous section (call it `enemy_orbital.tscn`) and change its script, or just change the script in the same scene to test.
...
> **Tip:** You don't need to look for another sprite. You can duplicate your chopper scene (`enemy_orbital.tscn`) and replace its script with this new one.
...
1.  Open `world.tscn`.
2.  Duplicate your `enemy.tscn` scene as `enemy_wave.tscn`.
3.  Modify the script with the wave code.
4.  Drag `enemy_wave.tscn` to the viewport.
