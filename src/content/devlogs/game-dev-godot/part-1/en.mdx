...
4.  The node will complain with an alert ⚠️. It's telling you: *"Hey, I have a physical body but I don't have a shape! Am I a circle? A square?"*.
...
1.  **`extends` (Inheritance)**:
    It's the first line. Tells Godot: *"I am an extension of..."*.
    *   `extends CharacterBody2D`: Means "I have all the methods and properties of a CharacterBody2D, plus whatever I add here".
    
    This is called **Class Inheritance**. It's a fundamental concept in object-oriented programming. Your script doesn't start from scratch: it inherits a "vehicle" that already works with wheels, engine, and steering (`CharacterBody2D`), and you just add the customization (your movement logic).
2.  **`func` (Action/Verb)**:
...
<Callout type="warning" title="Danger: Tabs vs Spaces">
  ...
  *   Always use **Tabs** (Tab Key ↹), not spaces.
...
<Callout type="info" title="Gamer Dictionary 🤓">
...
3.  You will see that it appears with a clapperboard icon 🎬 next to it. That means it is an **Instance**. If you change the original `player.tscn` (e.g., paint it red), this one will also change!
