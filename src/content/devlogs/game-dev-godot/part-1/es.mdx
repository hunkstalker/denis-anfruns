...
![Nave del Jugador](/assets/godot-basics/plane_128.png)
...
### 3. Glosario de Supervivencia

Antes de escribir código, necesitas saber leer el mapa. Si nunca has programado, esto es todo lo que necesitas saber por ahora. Imagina que el script es una receta de cocina:

1.  **`extends` (ADN)**:
    Es la primera línea. Le dice a Godot: *"Oye, soy una copia mejorada de..."*.
    *   `extends CharacterBody2D`: Significa "Tengo todos los superpoderes de un CharacterBody2D (chocar, moverse) y, además, lo que yo añada aquí".
2.  **`func` (Acción/Verbo)**:
...
### 4. Creando el Cerebro (Script)

Nuestra nave es bonita, pero tonta. No sabe qué hacer. Necesitamos un **Script**.
...
func _physics_process(delta):
    # Pedimos al Input que nos construya el Vector de Dirección basado en las teclas
