...
<Callout type="tip" title="¿Necesitas gráficos?">
  Para seguir este curso, necesitarás una nave. Si no tienes una a mano, te recomendamos encarecidamente el pack gratuito de **[Kenney Space Shooter Extension](https://kenney.nl/assets/space-shooter-extension)**. Es limpio, bonito y perfecto para aprender.
  
  ¿Necesitas editar las imágenes? No hace falta piratear nada. Tienes la suite **[Affinity Studio](https://www.affinity.studio/)** disponible de forma gratuita en la web.
</Callout>
...
En la intro dijimos que "Todo es una Escena". Cierto, pero... ¿de qué está hecha una escena? **De Nodos**.
...
![Nave del Jugador](/assets/gamedev-godot-basics/plane_128.png)

> Clic derecho -> Guardar imagen.  
> *Créditos: <a href="https://puszke.itch.io/psx-plane" target="_blank" rel="noopener noreferrer">PSX Plane</a> por **Puck**.*
...
1.  **`extends` (Herencia)**:
    Es la primera línea. Le dice a Godot: *"Soy una extensión de..."*.
    *   `extends CharacterBody2D`: Significa "Tengo todos los métodos y propiedades de un CharacterBody2D, más lo que yo añada aquí".
...
3.  **`var` (Variable)**:
    Es un lugar donde guardamos información **en la memoria RAM** del ordenador.
...
<Callout type="warning" title="Peligro: Tabs vs Espacios">
  GODOT NO PERDONA.
  La "sangría" (esos espacios a la izquierda) define qué código está dentro de qué función.
...
<Callout type="warning" title="¡Espera! ¿Qué es move_right?">
  Por defecto, Godot usa `ui_right`, `ui_left`, etc. para las flechas del teclado.
  Pero nosotros vamos a usar nombres profesionales ("move_right", "move_left") para poder asignar **WASD y Joystick** a la vez.

  1.  Ve a **Proyecto > Configuración del Proyecto > Mapa de Entradas**.
  2.  Añade las acciones: `move_right`, `move_left`, `move_up`, `move_down`.
  3.  Asigna sus teclas (WASD) y los ejes de tu mando favorito.
</Callout>
...
Tu script "hereda" de CharacterBody2D. Esto significa que tienes acceso a `velocity` (una variable interna) y `move_and_slide()` (un método que hace la magia de mover y chocar). Sin esta línea, GDScript no sabría qué eres.

**2. `func _physics_process(delta):`**

Godot llama esta función automáticamente **60 veces por segundo** (o el tick rate que configures). Todo lo que esté dentro se ejecuta en bucle. El parámetro `delta` contiene cuánto tiempo ha pasado desde el último frame (en segundos) – lo usaremos más adelante para hacer que todo vaya a la misma velocidad en cualquier PC.
...
Pero, **¿y si atraviesas la calle en diagonal?**
Aquí entra el señor Pitágoras con su famoso **Teorema**. No te asustes, vamos a diseccionarlo pieza a pieza:
...
Vamos a sustituir las letras por nuestros números (1 derecha, 1 abajo):
...
<Callout type="info" title="Diccionario Gamer 🤓">
  En videojuegos y matemáticas, a esta longitud $h$ la llamamos **Magnitud** o **Módulo** del vector.
  También es conocida como **Distancia Euclidiana** (la línea recta entre dos puntos).
  
  Cuando hacemos `input_vector.length()` en Godot, el motor hace este cálculo de Pitágoras por ti.
</Callout>

¡Ahí lo tienes!
Si no hacemos nada, tu dirección tiene una fuerza de **1.41**.
Comparado con la fuerza de **1.0** cuando vas recto, **¡estás corriendo un 41% más rápido!**
...
Es importante saberlo porque en matemáticas de shaders o IA, tendrás que usar `.normalized()` manualmente muchas veces cuando solo te importe la **dirección** ($ \hat{v} $) y no la fuerza.
...
Imagina que tu juego va a **60 FPS** (imágenes por segundo). Tu código se ejecuta 60 veces por segundo.
Ahora imagina que a tu amigo le va a **144 FPS**. Su código se ejecuta 144 veces por segundo.
...
2.  Elige **2D Scene** (Node2D). Esta vez no queremos físicas, solo un contenedor para el mundo. Llámalo `World` (PascalCase, porque es un Nodo).
3.  Guárdala en `scenes/world.tscn` (snake_case, porque es un archivo).
...
2.  **Arrástralo** desde el explorador de archivos y suéltalo dentro del panel de **Escena** (arriba a la izquierda, bajo donde dice `World`). Esto lo colocará automáticamente en la posición `(0, 0)`, perfectamente centrado.
...
1.  En la escena `World`, añade un nodo hijo **`Camera2D`**.
...
#### Centrando la Nave
Ahora que el (0,0) es la esquina, tu nave (que está en 0,0) se verá cortada arriba a la izquierda.
Vamos a colocarla en el centro exacto usando lógica:

1.  La resolución por defecto de Godot es **1152 x 648**.
2.  El centro es la mitad: `1152 / 2 = 576` (X) y `648 / 2 = 324` (Y).
3.  Selecciona el nodo `player` (la instancia).
4.  En Inspector > Transform > Position, escribe:
    *   **X**: `1152/2`
    *   **Y**: `648/2`

<Callout type="tip" title="Truco: ¡Godot sabe matemáticas!">
  Puedes escribir literalmente `1152/2` en la casilla y él hará la cuenta.
</Callout>
...
<Callout type="tip" title="¿Mi nave se va de la pantalla?">
  ¡Es normal! Aún no hemos puesto límites (paredes). Tu nave viaja felizmente hacia el infinito de coordenadas matemáticas.
  En próximos capítulos le pondremos muros a este universo.
</Callout>
