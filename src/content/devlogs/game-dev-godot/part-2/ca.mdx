...
![Sprite de la Bala](/assets/gamedev-godot-basics/shot.png)
...
*   **`preload("ruta")`**: Carrega l'escena **quan el joc arrenca**. És més ràpid en execució perquè ja està en memòria, però consumeix RAM des de l'inici.
*   **`load("ruta")`**: Carrega l'escena **sota demanda**, en el moment exacte que s'executa aquesta línia. Estalvia RAM inicial, però pot causar estrebades (micro-parades) si el recurs és pesat.

Per a bales (arxius petits que farem servir constantment), `preload` és l'elecció correcta.
