...
export default function SearchResultsClient({
	...
	const renderItem = (item: SearchResult) => (
		<a
			...
			className={
				isDesktop
					? 'group block h-full min-h-[140px] overflow-hidden rounded-xl bg-white/90 p-4 ring-1 ring-zinc-200/50 transition-all duration-200 ease-out hover:bg-white hover:shadow-lg hover:ring-[--tangerine] dark:bg-zinc-900/90 dark:ring-zinc-700/50 dark:hover:bg-zinc-800 dark:hover:ring-[--tangerine]'
					: 'group block rounded-xl bg-white/90 p-3 ring-1 ring-zinc-200/50 transition-all duration-200 ease-out hover:bg-white hover:shadow-lg hover:ring-1 hover:ring-[--tangerine] dark:bg-zinc-900/90 dark:ring-zinc-700/50 dark:hover:bg-zinc-800 dark:hover:ring-[--tangerine]'
			}
		>
			<div
          ...
          (contenido depurado en el momento de la compilación). */}
			<div
				className={`text-zinc-500 dark:text-zinc-400 ${isDesktop ? 'text-xs mt-2 line-clamp-5' : 'text-xs mt-1 line-clamp-1'}`}
	...
	return (
		<div className={rootClass}>
			<div className={scrollContainerClass}>
				{/* Mobile Chips */}
				{!isDesktop && (
					<div className="scrollbar-hide flex gap-2 overflow-x-auto pb-4 pt-2">
						{chips.map((chip) => (
							<button
								key={chip.id}
								onClick={() => setFilter((current) => (current === chip.id ? 'all' : chip.id))}
								className={`filter-chip text-xs whitespace-nowrap rounded-full px-4 py-1.5 font-medium transition-all ${
									filter === chip.id
										? 'active bg-[--tangerine] text-zinc-900'
