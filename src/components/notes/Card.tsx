...
// Get decorative icons for a Note (can return multiple for fan effect)
function getDecorativeIcons(post: NotePost): string[] {
	const icons: string[] = []
...
export default function TilCard({ post, lang, labels, badgeVariant = 'solid' }: Props) {
	...
	const href = lang === 'es' ? `/notes/${cleanSlug}/` : `/${lang}/notes/${cleanSlug}/`
	...
	return (
		<article className="note-card group relative flex h-full min-h-[233px] flex-col gap-3 overflow-hidden rounded-lg border border-zinc-200 bg-stone-100 p-4 shadow-sm transition-all duration-300 hover:scale-[1.02] hover:border-black/15 hover:shadow-md dark:border-zinc-700/50 dark:bg-zinc-900 dark:hover:border-white/15 sm:p-6">
				...
				<div className="pointer-events-none absolute -bottom-8 -right-12 z-0">
					{decorativeIcons.map((svg, index) => {
						...
						return (
							<div
								key={index}
								className="icon-fan absolute size-44 opacity-[0.30] transition-all duration-500 ease-out group-hover:scale-110 group-hover:opacity-[0.5] dark:opacity-[0.30] dark:group-hover:opacity-[0.50] [&_svg]:size-full"
								style={
									{
