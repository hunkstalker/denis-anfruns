...
const metaFiles = import.meta.glob('../content/devlog/**/meta.json', { eager: true, import: 'default' })
...
export async function getDevlogs(): Promise<DevlogPost[]> {
	...
	return posts
		.map((post) => {
            ...
            const meta = metaKey ? (metaFiles[metaKey] as { 
                pubDate?: string; 
                updatedDate?: string; 
                tags?: string[];
                draft?: boolean;
		...
		.filter((post): post is DevlogPost => post.data.pubDate !== undefined && post.data.pubDate !== null)
