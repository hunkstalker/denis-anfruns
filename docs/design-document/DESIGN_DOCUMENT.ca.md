...
- **Disseny Màgic (Magic Layout)**:
    - En filtrar o reordenar, els elements s'han de *lliscar* a les seves noves posicions, no saltar. Fes servir la propietat `layout` de `framer-motion`.
...
---
*Adhereix-te a aquestes regles per a tots els components futurs per mantenir la integritat del disseny del lloc.*
